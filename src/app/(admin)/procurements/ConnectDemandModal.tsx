...
interface ConnectDemandModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export default function ConnectDemandModal({ isOpen, onClose, onSuccess, purchaseId, oldDemandId, isClosed = false }: ConnectDemandModalProps) {
  ...
  const fetchDemands = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}app/fetchAvailableDemands?purchase_id=${purchaseId}`, {
        method: 'GET',
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      formData.append('old_demand_id', oldDemandId || '');
      formData.append('new_demand_id', selectedDemandId);

      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}app/changeDemandConnection`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: formData
      });
      ...
      <div className="bg-[#232b3e] border border-gray-700 shadow-2xl flex flex-col w-[800px] max-w-[95vw] h-[80vh] max-h-[80vh] rounded-xl overflow-hidden relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-t border-gray-700 bg-[#1b202c] shrink-0 flex justify-end gap-3">
          ...
          <button
            ...
            disabled={!selectedDemandId || isSubmitting || isLoading || isClosed}
