...
export default function UtilityTagsPage() {
  ...
  // API Call to fetch utility tags
  const fetchUtilityTags = useCallback(async (page: number) => {
    setIsLoading(true);
    try {
      ...
      const isSuccess = data && (
        ...
        data.Status === 1 || 
        (String(data.Status) === '0' && Array.isArray(data.tags_data))
      );

      if (isSuccess) {
        toast.success(data.Message || 'Utility Tags fetched successfully', { id: 'fetch-tags-toast' });
        
        if (data.tags_data && Array.isArray(data.tags_data)) {
          setTags(data.tags_data);
        } else {
          setTags([]);
        }
        ...
        toast.error(data.Message || 'Failed to fetch utility tags', { id: 'fetch-tags-toast' });
        setTags([]);
        setTotalPages(1);
      } else {
      ...
      console.error('Fetch utility tags error:', e);
      toast.error(e.message || 'Error fetching utility tags', { id: 'fetch-tags-toast' });
      setTags([]);
      setTotalPages(1);
    } finally {
      setIsLoading(false);
  ...
  // Outside Click handler for search suggestions dropdown
  useEffect(() => {
  ...
  // Debounced API search recommendation hook
  useEffect(() => {
    ...
    const performSearch = async () => {
       ...
       try {
         ...
         if (data && (String(data.Status) === '1' || data.Status === 1) && data.tags_data && Array.isArray(data.tags_data)) {
           setSearchResults(data.tags_data);
  ...
  }, [searchQuery, fetchUtilityTags]);

  const handleSelectSearchResult = (selectedTag: any) => {
    setShowSearchDropdown(false);
    ...
    setSearchQuery(selectedTag.utility_tag);
    lastSearchedQuery.current = selectedTag.utility_tag;
    ...
    toast.success(selectedTag.utility_tag + ' loaded successfully', { id: 'search-select-toast' });
  };
  ...
  // Load tags on filter or page change
  useEffect(() => {
    fetchUtilityTags(1);
  }, [fetchUtilityTags, filterStatus]);

  const handlePageChange = (newPage: number) => {
    if (newPage > 0 && newPage <= totalPages) {
      fetchUtilityTags(newPage);
    }
  };

  const handleReload = () => {
    fetchUtilityTags(currentPage);
  };

  const handleToggleAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedTagIds(tags.map(t => String(t.tag_id)));
    } else {
      setSelectedTagIds([]);
    }
  };
  ...
  // Get active tag for detail overlay fallbacks
  const getSelectedTagObj = (id: string | null) => {
    if (!id) return undefined;
    return tags.find(t => String(t.tag_id) === id);
  };
  ...
  const executePatch = async () => {
    ...
    try {
      ...
      formData.append('tags_csv', confirmDialog.payload.tags_csv);
      formData.append('status', confirmDialog.payload.status);
      ...
      if (data && (String(data.Status) === '1' || data.Status === 1)) {
        toast.success(data.Message || 'Utility Tag status updated successfully', { id: 'patch-tags-toast' });
        setSelectedTagIds([]);
        setConfirmDialog(null);
        fetchUtilityTags(currentPage);
      } else {
        toast.error(data?.Message || 'Failed to update utility tag status', { id: 'patch-tags-toast' });
      }
    } catch (e: any) {
      console.error('Patch utility tags error:', e);
      toast.error(e.message || 'Error updating status', { id: 'patch-tags-toast' });
    } finally {
      setIsPatching(false);
    }
  };

  // Handler for row-level switch toggles
  const handleStatusToggleRequest = (tag: any) => {
    const isStatusActive = String(tag.active).toLowerCase() === "yes" || String(tag.active) === "1";
    const newStatus = isStatusActive ? "0" : "1";
    const statusText = newStatus === "1" ? "Enable" : "Disable";
    
    setConfirmDialog({
       isOpen: true,
       title: `Change Status to ${statusText} for tag "${tag.utility_tag}"?`,
       payload: {
         tags_csv: String(tag.tag_id),
         status: newStatus
       }
    });
  };

  // Handler for bulk status dropdown selects
  const handleBulkStatusChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setConfirmDialog({
      isOpen: true,
      title: `Change Status to ${statusText} for ${selectedTagIds.length} selected record(s)?`,
      payload: {
        tags_csv: selectedTagIds.join(','),
        status: newStatus
      }
    });
  ...
  // Search query results are handled directly via API injection and loaded to tags state
  ...
  return (
    <div className="p-6 text-gray-300 bg-[#11141e] min-h-full">
      ...
      <div className="flex items-center gap-3 mb-6">
        <h1 className="text-xl font-bold text-white tracking-wide">Utility Tags</h1>
        <RefreshCcw 
      ...
      <div className="bg-[#191e2b] border border-gray-800 rounded-xl shadow-sm overflow-hidden flex flex-col h-[calc(100vh-140px)]">
              ...
              <div className="absolute top-[38px] left-0 right-0 bg-[#1f2536] border border-gray-600 rounded-md shadow-2xl z-50 max-h-64 overflow-y-auto overflow-x-hidden">
                <ul className="py-1">
                  {searchResults.map((result) => (
                    ...
                    >
                      <div className="font-medium truncate">{result.utility_tag}</div>
                      {result.description_txt && <div className="text-[11px] text-gray-400 mt-0.5 truncate">{result.description_txt}</div>}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="p-4 border-b border-gray-800 flex flex-col md:flex-row md:justify-between md:items-center bg-[#191e2b] gap-4 shrink-0">
          ...
          {/* New Tag Trigger Button */}
          <button 
             onClick={() => { setEditTagId(null); setIsEditModalOpen(true); }}
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
             <thead className="text-[11px] text-gray-400 font-semibold uppercase bg-[#191e2b] border-b border-gray-800 sticky top-0 z-10">
               <tr>
                 ...
                 <th className="px-4 py-3.5">Utility Tag Name</th>
               ...
               {isLoading ? (
                  <tr>
                     <td colSpan={6} className="py-12 text-center text-gray-500">
                       ...
                       Loading Utility Tags Database...
                     </td>
                  </tr>
               ) : tags.length === 0 ? (
                  <tr>
                     <td colSpan={6} className="py-12 text-center text-gray-500 italic">
                       No utility tags located matching search parameters.
                     </td>
                  </tr>
               ) : (
                  tags.map((tag: any) => {
                     const isStatusActive = String(tag.active).toLowerCase() === "yes" || String(tag.active) === "1";
                     return (
                       <tr key={tag.tag_id} className="hover:bg-[#1f2536] transition-colors group">
                         <td className="px-5 py-3.5">
                           <input 
                             type="checkbox" 
                             checked={selectedTagIds.includes(String(tag.tag_id))}
                             onChange={() => handleToggleIndividual(String(tag.tag_id))}
                         ...
                         <td className="px-4 py-3.5 font-medium text-[#cbd5e1] max-w-[300px] truncate" title={tag.description_txt}>
                           {tag.description_txt || '-'}
                         </td>
                         <td className="px-4 py-3.5 text-center">
                            <label className="relative inline-flex items-center cursor-pointer">
                              <input 
                                ...
                                onChange={() => handleStatusToggleRequest(tag)}
                              />
                        ...
                        <td className="px-4 py-3.5 text-center">
                          <button 
                            onClick={() => setViewTagId(String(tag.tag_id))} 
                        ...
                        <td className="px-4 py-3.5 text-center">
                          <button 
                            onClick={() => { setEditTagId(String(tag.tag_id)); setIsEditModalOpen(true); }} 
        ...
        <div className="p-4 border-t border-gray-800 text-xs text-gray-500 flex justify-between items-center bg-[#191e2b] shrink-0">
            <span>Page {currentPage} of {totalPages} ({tags.length} rows loaded on current page)</span>
      ...
      {/* Modal Dialog Panels */}
      <ViewTag 
         isOpen={viewTagId !== null}
         onClose={() => setViewTagId(null)}
         tagId={viewTagId}
         fallbackData={getSelectedTagObj(viewTagId)}
      />
      ...
      <EditTag 
         ...
         tagId={editTagId}
         fallbackData={getSelectedTagObj(editTagId)}
