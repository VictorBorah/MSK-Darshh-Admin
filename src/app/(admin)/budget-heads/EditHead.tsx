...
export default function EditHead({ isOpen, onClose, headData, onSuccess }: EditHeadProps) {
  ...
  useEffect(() => {
    if (isOpen && headData) {
      setHeadName(headData.head || '');
      setDefaultGst(headData.default_gst || '0.00');
      setHasVendor(String(headData.has_vendor) === '1');
      setStatusToggle(String(headData.active) === '1');
    }
  }, [isOpen, headData]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      payload.append('heads_csv', String(headData.id));
      ...
      if (response && (String(response.Status) === '1' || response.Status === 1)) {
        toast.success(response.Message || response.message || 'Budget Head successfully updated');
      ...
      <div className="bg-[#1f2536] border border-gray-700 rounded-lg shadow-xl w-full max-w-lg p-0 overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSave} className="flex flex-col flex-1">
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-center justify-between border border-gray-700 p-3 rounded-md">
              <div className="flex flex-col">
                <span className="text-[13px] font-medium text-gray-300 uppercase tracking-wide">Status</span>
                <span className="text-[11px] text-gray-500">Toggle active state for this budget head</span>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input 
                  type="checkbox" 
