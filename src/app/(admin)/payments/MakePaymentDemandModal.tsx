...
interface MakePaymentDemandModalProps {
  isOpen: boolean;
  onClose: () => void;
  projects: any[];
  priorities: any[];
...
export default function MakePaymentDemandModal({ isOpen, onClose, projects, priorities, onSuccess }: MakePaymentDemandModalProps) {
  ...
  const [configureQnty, setConfigureQnty] = useState<number | ''>('');
  const [configureAmount, setConfigureAmount] = useState<string>('');
  ...
  useEffect(() => {
    ...
    const performSearch = async () => {
      ...
      try {
        ...
        const endpoint = `${process.env.NEXT_PUBLIC_API_BASE_URL}app/searchItem?query_str=${encodeURIComponent(itemSearch)}&project_id=${selectedProject}&is_material=0`;
  ...
  const handleOpenConfigure = async (item: any) => {
    ...
    setConfigureQnty(item.qnty);
    setConfigureAmount(item.amount);
    ...
    try {
      ...
      if (String(data.Status) === "1" && data.autogen_title) {
         setConfigureTitle(data.autogen_title);
         if (!item.comment) {
           setConfigureComment(data.autogen_title);
         }
         toast.success(data.Message || "Title generated successfully");
      } else if (String(data.Status) === "0") {
         toast.error(data.Message || "Failed to auto-generate title");
      }
    } catch (err) {
  ...
  const handleQuantityBlur = async () => {
    if (!configuringItem || !selectedProject || !configureQnty || isAutoGenerating) return;

    setIsAutoGenerating(true);
    try {
      const token = localStorage.getItem('at_ki8Xq1iV');
      const endpoint = `${process.env.NEXT_PUBLIC_API_BASE_URL}app/getAutogenDemandTitle?project_id=${selectedProject}&item_id=${configuringItem.id}&qnty=${configureQnty}`;
  ...
  const handleSaveConfigure = () => {
    if (configuringItem) {
      ...
      if (idx !== -1) {
         ...
         newItems[idx].qnty = configureQnty;
         newItems[idx].amount = configureAmount;
         setItemsList(newItems);
      }
    }
  ...
  const handleConfirmDemand = async () => {
    ...
    const demandJson = itemsList.map(item => ({
      ...
      payment_amount: String(item.amount),
  ...
  const grandTotal = itemsList.reduce((sum, item) => sum + ((Number(item.qnty) || 0) * (Number(item.amount) || 0)), 0);
  const isConfirmDisabled = isSaving || itemsList.length === 0 || itemsList.some(item => !item.qnty || Number(item.qnty) <= 0 || !item.amount || Number(item.amount) <= 0);
        ...
        >
          ...
          <div className="px-5 py-4 border-b border-gray-700 flex justify-between items-center bg-[#293653] shrink-0">
            <div className="flex items-center gap-6">
              <h2 className="text-[16px] text-white font-bold tracking-wide flex items-center gap-2">
                <ClipboardList className="w-5 h-5" /> New Payment Demand
              </h2>
          ...
          <div className="flex-1 overflow-y-auto bg-[#232b3e] scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
            <table className="w-full text-left">
              <thead className="text-[12px] text-white font-bold uppercase bg-[#3f4a60] sticky top-0 z-10 shadow-sm">
                <tr>
                  <th className="px-6 py-2.5 w-12">SL</th>
                  <th className="px-6 py-2.5 flex-1">ITEM</th>
                  <th className="px-6 py-2.5 w-28">QNTY</th>
                  <th className="px-6 py-2.5 w-24">UNIT</th>
                  <th className="px-6 py-2.5 w-32">AMOUNT (₹)</th>
                  <th className="px-6 py-2.5 w-28 text-center">CONFIGURE</th>
                  <th className="px-6 py-2.5 w-20 text-center">REMOVE</th>
                </tr>
              </thead>
                  ...
                  <tr key={item.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4 text-white font-normal text-[12px]">
                      <input
                        ...
                        className="w-16 bg-[#161a25] border border-gray-600 focus:border-blue-500 rounded px-2 py-1 text-white text-[12px] font-medium outline-none transition-colors"
                        min="1"
                      />
                    </td>
                    ...
                    <td className="px-6 py-4 text-white font-normal text-[12px]">
                      <input
                        ...
                        className="w-24 bg-[#161a25] border border-gray-600 focus:border-blue-500 rounded px-2 py-1 text-white text-[12px] font-medium outline-none transition-colors"
                        min="0"
                      />
                    </td>
          ...
          <div className="px-6 py-4 bg-[#1b202c] border-t border-gray-800 flex flex-col items-end gap-4 shrink-0">
            <div className="flex gap-6 text-[12px] text-white font-normal tracking-wide">
              <div>Total Items: <span className="ml-1 font-semibold">{totalItems}</span></div>
              <div>Grand Total: <span className="ml-1 font-semibold text-green-400">₹{grandTotal.toFixed(2)}</span></div>
            </div>
            ...
            <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
              <div className="bg-[#232b3e] border border-gray-700 rounded-xl shadow-2xl flex flex-col overflow-hidden w-[500px] max-w-[95vw]">
                ...
                <div className="p-6 flex flex-col gap-4 bg-[#1b202c]">
                  {configuringItem && (
                    <div className="text-[13px] text-gray-300 border-b border-gray-700/50 pb-3 flex justify-between items-center">
                      <span>Configuring for <span className="font-semibold text-white">{configuringItem.name}</span></span>
                    </div>
                  )}
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[13px] font-medium text-gray-300 mb-1.5">Quantity</label>
                      <input
                        type="number"
                        value={configureQnty}
                        onChange={(e) => setConfigureQnty(e.target.value === '' ? '' : parseInt(e.target.value, 10))}
                        onBlur={handleQuantityBlur}
                        min="1"
                        className="w-full bg-[#161a25] border border-gray-600 rounded px-3 py-2 text-white text-[13px] focus:outline-none focus:border-blue-500 transition-colors"
                      />
                    </div>
                    <div>
                      <label className="block text-[13px] font-medium text-gray-300 mb-1.5">Amount (₹)</label>
                      <input
                        type="number"
                        step="0.01"
                        value={configureAmount}
                        onChange={(e) => setConfigureAmount(e.target.value)}
                        min="0"
                        className="w-full bg-[#161a25] border border-gray-600 rounded px-3 py-2 text-white text-[13px] focus:outline-none focus:border-blue-500 transition-colors"
                      />
                    </div>
                ...
                <div className="px-6 py-4 bg-[#293653] border-t border-gray-700 flex items-center justify-between gap-3">
                  <div className="text-[13px] text-white font-medium">
                    Grand Total: <span className="text-green-400 font-bold ml-1">₹{((Number(configureQnty) || 0) * (Number(configureAmount) || 0)).toFixed(2)}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      disabled={isAutoGenerating || !configureTitle.trim() || !configureQnty || Number(configureQnty) <= 0 || !configureAmount || Number(configureAmount) <= 0}
