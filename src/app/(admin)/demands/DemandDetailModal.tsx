import { X, Maximize2, Minimize2, ClipboardList, Settings, Loader2, Trash2, Lock } from 'lucide-react';
...
export default function DemandDetailModal({ isOpen, onClose, demandNo, priorities, onSuccess }: DemandDetailModalProps) {
  ...
  const handleOpenConfigure = async (item: any) => {
    ...
    setIsLocked(String(item.is_locked) === '1' || item.is_locked === 1 || item.is_locked === 'Yes');
             ...
             <table className="w-full text-[12px] text-left">
                ...
                <tbody className="divide-y divide-gray-700">
                  {demandsData.map((row, idx) => {
                    ...
                    return (
                      <tr key={row.demand_id || row.id} className={`transition-colors ${isRowLocked ? 'bg-red-950/40 hover:bg-red-950/50' : 'hover:bg-white/5'}`}>
                        ...
                        <td className="px-4 py-3 text-center">
                          <button 
                            ...
                            disabled={removingId === String(row.demand_id || row.id) || isRowLocked}
                            title={isRowLocked ? 'Demand Locked' : 'Remove'}
