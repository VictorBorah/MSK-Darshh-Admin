...
export default function TaxationDetailsModal({ isOpen, onClose, item, vendors, demands }: TaxationDetailsModalProps) {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      await fetchTaxation(item.vendor_id || '', item.item_id, item.qnty || '1', gstInc, '', '');
               ...
               <div className="flex flex-col gap-6">
                   <div className="bg-[#1b202c] p-5 border border-gray-700 rounded-lg shadow-inner">
                       <div className="flex flex-col gap-2">
                         ...
                         <Select
                           options={demands?.map((d: any) => ({ value: String(d.demand_no), label: `[#${d.demand_no}] ${d.demand_title || d.item_name || 'Demand'}` })) || []}
                           value={demands?.find(d => String(d.demand_no) === connectDemand) ? { value: connectDemand, label: `[#${connectDemand}] ${demands.find((d: any) => String(d.demand_no) === connectDemand)?.demand_title || demands.find((d: any) => String(d.demand_no) === connectDemand)?.item_name || 'Demand'}` } : null}
                           onChange={(val: any) => {
                   ...
                   {demandDetails.length > 0 ? (
                     <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300 h-full">
                       <h3 className="text-[13px] font-semibold text-white border-b border-gray-700/50 pb-2 flex items-center gap-2 shrink-0">
                         <LinkIcon className="w-4 h-4 text-blue-400" /> Connected Demand Details
                       </h3>
                                   ...
                                   <td className="px-4 py-3 align-top">
                                     ...
                                     <div className="mt-1.5">
                                       <span className={`px-2 py-0.5 rounded text-[10px] font-medium border ${
                                          String(dRow.priority_id) === '1' ? 'bg-red-500/10 text-red-500 border-red-500/20' :
