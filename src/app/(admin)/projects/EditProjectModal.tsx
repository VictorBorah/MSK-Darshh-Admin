  ...
  const fetchAggregatedData = async () => {
    ...
    try {
      ...
      if (pData && String(pData.Status) === '1') {
        ...
        setTmId(mapHrIds(pd.technical_managers_data, cData.hr_array?.technical_managers));
        setAssttAdminId(mapHrIds(pd.assistant_admins_data, cData.hr_array?.asstt_admins));
                  ...
                  <div className="flex flex-col">
                    ...
                    <FormRow label="Start Date" helpText="Date of commencement of construction">
                      <input
                        ...
                        className={`w-full bg-[#eee0e0] border-none text-gray-900 text-[13px] font-medium rounded-sm px-2.5 h-8 focus:outline-none cursor-pointer light-bg-date-picker ${errorField === 'startDate' ? 'ring-2 ring-red-500 bg-red-100' : 'focus:ring-2 focus:ring-emerald-500'}`}
                      />
                    </FormRow>
