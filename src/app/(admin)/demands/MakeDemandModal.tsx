        ...
        <div className="px-6 pt-4 pb-7 bg-[#1b202c] border-b border-gray-800 grid grid-cols-1 md:grid-cols-2 gap-6 items-center shrink-0">
          <div className="flex items-center gap-4">
             ...
             <div className="flex-1">
               <Select
                 options={projects.map((p: any) => ({ value: String(p.id || p.project_id), label: p.project_name }))}
                 onChange={(val: any) => {
                 ...
                 value={projects.find(p => String(p.id || p.project_id) === selectedProject) ? { value: selectedProject, label: projects.find((p: any) => String(p.id || p.project_id) === selectedProject)?.project_name } : null}
