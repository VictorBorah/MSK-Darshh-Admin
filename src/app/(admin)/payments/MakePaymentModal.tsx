...
export default function MakePaymentModal({ isOpen, onClose, projects, paymentModes, demands, onSuccess }: MakePaymentModalProps) {
  ...
  const checkActiveDemands = async (projectId: string) => {
    ...
    try {
      const token = localStorage.getItem('at_ki8Xq1iV');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}app/getDemandInformation?project_id=${projectId}&is_material=0`, { headers: { 'Authorization': `Bearer ${token}` } });
      const text = await res.text();
      let arr; try { arr = JSON.parse(text); } catch (e) { }
      const data = arr && Array.isArray(arr) ? arr[0] : arr;
  ...
  const handleLoadDemands = async () => {
    ...
    try {
      const token = localStorage.getItem('at_ki8Xq1iV');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}app/fetchDemandItems?project_id=${selectedProject}&is_material=0`, { headers: { 'Authorization': `Bearer ${token}` } });
      const text = await res.text();
      let arr; try { arr = JSON.parse(text); } catch (e) { }
      const data = arr && Array.isArray(arr) ? arr[0] : arr;
      ...
      if (data && String(data.Status) === '1' && data.items_data) {
        ...
        data.items_data.forEach((item: any) => {
          ...
          if (!isDuplicate) {
            ...
            const default_val = parseFloat(item.payment_amount || '0');
            const newItem = {
              ...
              isCalculatingTds: true,
              needsTdsCalc: true,
              showTdsAlertTrigger: true
            };
    ...
    setTableItems(prev => prev.map(item => {
      if (item.id === id) {
        ...
        if (field === 'qnty' || field === 'price') {
          updated.needsTdsCalc = true;
          updated.isCalculatingTds = true;
        }
        return updated;
      }
      return item;
  ...
  useEffect(() => {
    const itemsToCalc = tableItems.filter(item => item.needsTdsCalc);
  ...
  return (
    <>
      ...
      <SuccessConfirmationModal
        ...
        content="Found active payment demands for this project. Load them automatically?"
      />

      <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 md:p-8">
        <div className={`bg-[#232b3e] border border-gray-700 rounded-xl shadow-2xl flex flex-col overflow-hidden relative transition-all duration-300 ${isMaximized ? 'w-full h-full fixed inset-0 m-0 rounded-none' : 'w-[1000px] max-w-[95vw] max-h-[90vh]'
          }`}>
                ...
                <table className="w-full text-[13px] text-left">
                  <thead className="bg-[#1b202c] text-gray-400 border-b border-gray-700">
                    <tr>
                      ...
                      <th className="px-4 py-3 font-semibold uppercase tracking-wider min-w-[180px]">Mode</th>
                      <th className="px-4 py-3 font-semibold uppercase tracking-wider w-28 border-l border-gray-700/50">Qnty</th>
                      <th className="px-4 py-3 font-semibold uppercase tracking-wider whitespace-nowrap min-w-[140px] border-l border-gray-700/50">Unit Payment</th>
                      ...
                      <tr key={row.id} className="hover:bg-white/5 transition-colors group">
                        ...
                        <td className="px-4 py-2">
                          <Select
                            ...
                            value={paymentModes?.find(m => String(m.id) === String(row.mode_id)) ? { value: String(row.mode_id), label: paymentModes.find((m: any) => String(m.id) === String(row.mode_id))?.mode } : null}
                            onChange={(val: any) => handleFieldChange(row.id, 'mode_id', val ? val.value : '')}
                        ...
                        <td className="px-4 py-2 border-l border-gray-700/50">
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full bg-[#191e2b] border border-gray-600 rounded pl-7 pr-2 py-1.5 text-white text-right focus:outline-none focus:border-blue-500 transition-colors"
                            />
                          </div>
                        </td>
                        <td className="px-4 py-3 border-l border-gray-700/50 font-medium text-emerald-400 text-right">
                          <div className="flex items-center justify-end gap-1">
                            {row.isCalculatingTds ? (
                      ...
                      <tr>
                        <td colSpan={7} className="px-4 py-16 text-center">
                          <div className="flex flex-col items-center justify-center gap-2 text-gray-500">
                            ...
                            <p>Search and select items to add to the payment list.</p>
                          </div>
                        </td>
                      </tr>
                    )}
