...
interface PaymentDemandDetailModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function PaymentDemandDetailModal({ isOpen, onClose, demandNo, priorities, onSuccess }: PaymentDemandDetailModalProps) {
  ...
  const [configureQuantity, setConfigureQuantity] = useState<number | ''>('');
  const [configureAmount, setConfigureAmount] = useState('');
  ...
  const handleOpenConfigure = async (item: any) => {
    ...
    setConfigureQuantity(item.quantity ? Number(item.quantity) : '');
    setConfigureAmount(String(item.payment_amount || '0'));
    ...
    if (!hasTitle || !hasDesc) {
      ...
      try {
        ...
        if (String(data.Status) === '1') {
          ...
          setOriginalConfig({ title: finalTitle, comment: finalDesc, quantity: String(item.quantity || ''), amount: String(item.payment_amount || '0'), priority: String(item.priority_id || '3') });
        } else {
  ...
  const handleQuantityBlur = async () => {
    if (!configuringItem || !configureQuantity || isLocked || isAutoGenerating) return;
    ...
    try {
      ...
      params.set('qnty', String(configureQuantity));
  ...
  const handleUpdateDemand = async () => {
    ...
    if (!configureQuantity || Number(configureQuantity) <= 0) {
      toast.error('Quantity is required');
      return;
    }
    if (!configureAmount || Number(configureAmount) <= 0) {
    ...
    try {
      ...
      payload.set('amount', configureAmount); // Explicit POST Param requested
  ...
  const configureGrandTotal = (Number(configureQuantity) || 0) * (Number(configureAmount) || 0);
  const isConfigureSaveDisabled = isLocked || isUpdatingDemand || isAutoGenerating || !configureTitle.trim() || !configureQuantity || Number(configureQuantity) <= 0 || !configureAmount || Number(configureAmount) <= 0 || (configureTitle === originalConfig.title && configureComment === originalConfig.comment && String(configureQuantity) === originalConfig.quantity && String(configureAmount) === originalConfig.amount && String(configurePriority) === originalConfig.priority);
    ...
    <div className={`fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 transition-all`}>
      <div 
        className={`bg-[#232b3e] border border-gray-700 rounded-xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 ${
          isMaximized ? 'w-full h-full fixed inset-0 m-0 rounded-none' : 'w-[950px] max-w-[95vw] max-h-[90vh]'
        }`}
      >
        {/* Header */}
                ...
                <tbody className="divide-y divide-gray-700">
                  {demandsData.map((row, idx) => (
                    <tr key={row.demand_id || row.id} className={`transition-colors ${row.is_locked === 'Yes' ? 'bg-red-500/10 hover:bg-red-500/20' : 'hover:bg-white/5'}`}>
                      ...
                      <td className="px-4 py-3 text-white font-medium text-green-400">₹{Number(row.payment_amount || 0).toFixed(2)}</td>
        ...
        <div className="px-6 py-4 bg-[#1b202c] border-t border-gray-800 flex flex-col items-end gap-4 shrink-0">
           <div className="flex gap-6 text-[12px] text-white font-normal tracking-wide">
              <div>Total Items: <span className="ml-1 font-semibold">{itemCount || demandsData.length}</span></div>
              <div>Grand Total: <span className="ml-1 font-semibold text-green-400">₹{demandsData.reduce((sum, item) => sum + ((Number(item.quantity) || 0) * (Number(item.payment_amount) || 0)), 0).toFixed(2)}</span></div>
           </div>
            ...
            <div className="bg-[#232b3e] border border-gray-700 rounded-xl shadow-2xl flex flex-col overflow-hidden w-[500px] max-w-[95vw]">
              ...
              <div className="p-6 flex flex-col gap-4 bg-[#1b202c]">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                   <div>
                     ...
                     <input
                       ...
                       onChange={(e) => setConfigureQuantity(e.target.value === '' ? '' : parseInt(e.target.value, 10))}
                 ...
                 <div>
                   ...
                   <Select
                     ...
                     isDisabled={isLocked || isAutoGenerating}
              ...
              <div className="px-6 py-4 bg-[#293653] border-t border-gray-700 flex items-center justify-between gap-3">
                 <div className="text-[13px] text-white font-medium">
                   Grand Total: <span className="text-green-400 font-bold ml-1">₹{configureGrandTotal.toFixed(2)}</span>
                 </div>
                 <div className="flex items-center gap-3">
                   ...
                   <button 
                     ...
                     disabled={isConfigureSaveDisabled}
